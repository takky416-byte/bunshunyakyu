...
- **予約日時は、実行時点から30日後までしか指定できません**(yakyu.bunshun.jp側の
  仕様。31日後以降を指定すると、送信ボタンを押してもページ遷移が起きず失敗する
  ことを実際に確認済み)。30日を超える日時を指定した場合はエラーで止まります。
  多数の記事をまとめて連日予約したい場合、一度に用意できるのは実行日から
  30日分までで、それより先の分は日付が近づいてから改めて実行してください。
