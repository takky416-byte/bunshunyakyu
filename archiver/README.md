...
太字/斜体/下線は Ctrl+B・Ctrl+I・Ctrl+U のショートカットを使って切り替えながら実際に
キー入力し、引用はTrixツールバーの「引用」ボタンを押した状態で入力することで反映して
います(`editor.insertHTML()` のようにJS側からTrix/Vueの内部状態を直接書き換える方式も
試しましたが、見た目上は正しく反映されても、その後の送信がエラーも出さず効かなくなる
現象が実際の動作確認で判明したため、本物のキー入力に近い方式に変更しています)。
本文中への画像挿入は、
...
- 画像(ヘッダー画像・本文中の画像)は、アップロードが完了する(一時的な
  `blob:` プレビューURLから実際のサーバーURLに置き換わる)まで待ってから
  送信するようにしています。アップロードが完了しないうちに送信すると、
  サイト側がエラーも出さず黙って送信を無視する(ボタンを押しても何も起きない)
  ことが実際に確認できたためです。回線が遅い場合など、既定の待ち時間(30秒)を
  超えると警告を出しますが、そのまま送信を試みます。チェック対象はヘッダー画像と
  本文エディタの中の画像だけに絞っています(ページ全体を見ると、投稿フォームと
  無関係な要素がたまたま `blob:` を使っていた場合に誤検知するため)。ただし画像を
  一切使わなくても送信が反応しないケースがあり、下記の理由(JS側からの内部状態
  書き換え)が主な原因だったため、このチェックだけでは不十分な場合があります。
- 太字/斜体/下線は Ctrl+B・Ctrl+I・Ctrl+U のキーボードショートカットで、引用は
  Trixツールバーの「引用」ボタンを押した状態で、それぞれ実際にキー入力すること
  で反映しています。単一の `*` や `_` を装飾以外の目的(地の文中の記号として)で
  使いたい場合は、意図せずイタリックの開始/終了と解釈されてしまう可能性があります。
