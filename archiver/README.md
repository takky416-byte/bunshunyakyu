...
キーボードショートカットの推測に頼らず確実に反映されます。本文中への画像挿入は、
...
- 画像(ヘッダー画像・本文中の画像)は、アップロードが完了する(一時的な
  ...
  送信するようにしています。実際の動作確認で、アップロードが完了しないうちに
  送信すると、サイト側がエラーも出さず黙って送信を無視する(ボタンを押しても
  何も起きない)ことが分かったためです。回線が遅い場合など、既定の待ち時間
  (30秒)を超えると警告を出しますが、そのまま送信を試みます。チェック対象は
  ヘッダー画像と本文エディタの中の画像だけに絞っています(ページ全体を見ると、
  投稿フォームと無関係な要素がたまたま `blob:` を使っていた場合に誤検知するため)。
