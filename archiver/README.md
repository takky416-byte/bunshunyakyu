...
- 太字/斜体は、本文入力時にエディタへ Ctrl+B / Ctrl+I ショートカットを送って
  切り替える仕組みです。投稿先のリッチテキストエディタがこれらのショートカットに
  対応していない場合、装飾は反映されません。
- 本文途中の画像挿入・ヘッダー画像設定は、いずれもエディタのツールバー上の
  「画像」ボタンやアップロード欄をテキスト(`NEW_POST_INLINE_IMAGE_BUTTON_TEXT` /
  `HEADER_IMAGE_LABEL_TEXT`)から推測して探す仕組みです。見出しの文言やボタンの
  構造が想定と違うと見つからずエラーになります。
- Markdownの見出し記法(`#`など)はそのまま文字として入力されます(太字/斜体/画像
  以外の記法はサポートしていません)。
- 予約投稿の切り替え・日時入力・送信ボタンの検出も文言/一般的な属性からの推測です。

いずれも、失敗時に自動保存される `error_form.html` / `error_form.png`
