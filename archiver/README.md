...
このツールはもともと「IT野球選手名鑑」専用でしたが、投稿先は同じ文春野球友の会サイト
(yakyu.bunshun.jp)上の新規投稿フォームであれば何でもよいため、今はブログの種類を
問わず使えます。ブログ(記事シリーズ)ごとにフォルダを分ければ、複数のブログを
並行して自動投稿できます(詳しくは後述の「複数のブログを自動投稿する」を参照)。
興味関心タグに既定値は無いので、ブログごとに `tags.txt` や `--tag` で明示的に
指定してください(誤って別のブログの記事に他ブログ用のタグが付かないようにするためです)。
...
- 見出し(本物の`<h3>`見出し): 段落の先頭を `# `(半角シャープ+半角スペース)にする
  (例: `# 弘治元年　安芸国・厳島`)。このサイトの本文エディタは見出しレベルを
  1つしか持たないため、`##`のように`#`を複数書いても同じ見出しになる。
  太字/斜体/下線の装飾は見出しには付けない。
...
- `**太字**`・`*斜体*`・`__下線__`(旧記法)も後方互換のため引き続き使えるが、
  **ChatGPT等に生成させる場合は`<b>`/`<i>`/`<u>`タグを使うことを推奨**する
  ...
  実際にあった)。
- 1つの段落(空行で区切られていない範囲)の中で改行したい場合は、`article.txt`側で
  単純に行を分けて書けばよい(明示的に`<br>`と書く必要はなく、自動的に`<br>`として
  扱われる)。プロフィール欄・関連項目の列挙・複数行の引用などはこの書き方でよい。
- `# `による本物の見出し、または段落全体が`<u>`(下線)だけで装飾された疑似見出しの
  直前は、通常の1行分ではなく**2行分**の空きになる(見出しの前後で区切りを
  分かりやすくするため)。
- 段落と段落の間(空行区切り)は、それ以外は実際のブログでは常に**空行1行分**の間隔になる
  (以前は実装の都合でEnterキーを押して段落を区切っていたが、キー入力のタイミング
  次第で区切りが全く効かず全文がつながってしまう不具合が実際にあったため、他の
  改行と同じ`<br><br>`挿入方式に統一して解消した)。
- 引用(`> `)が複数行にわたる場合も同様に自動で`<br>`扱いになる。継続行の先頭に
...
- **予約投稿は実行時点から30日後までしか設定できないようです**(文春野球友の会サイト側の
  仕様。ドキュメントで明文化されているわけではなく、実際の挙動に基づく制限です)。
  それを超える日時を指定すると、フォーム入力を始める前にエラーで教えてくれます
  (`post_blog.py` 内の `MAX_RESERVATION_DAYS` で調整可能)。1か月以上先の記事を
  まとめて予約したい場合は、投稿日が近づいてから改めて実行してください。
- 興味関心タグに既定値はありません(複数のブログを投稿できるようにする際、指定し忘れると
  無関係なブログの記事に別ブログ用のタグが付いてしまう問題を避けるため、既定のタグという
  概念自体を廃止しています)。単発投稿(`--batch`/`--batch-dir` を使わない場合)では、
  タグを付けたい場合は `--tag タグ名`(複数指定可、指定した分だけが使われる)、
  付けたくない場合は `--no-tags` を必ずどちらか指定してください(どちらも指定しないと
  エラーになります)。実際のUIは「興味関心タグ」欄の「編集する」ボタン→開いたモーダル内の
  検索ボックスにタグ名を入力して絞り込み→完全一致するカードをクリック→「完了」で確定、
  という流れになっており(似た名前のタグ、例:「偏愛選手名鑑2023」も候補に出るため完全一致で
  選ぶ必要がある)、この一連の操作を自動で行います。検索してもタグが候補に
  出てこない場合は指定しても設定できません。
...
- このフォームにはヘッダー画像のトリミング用UI(croppaライブラリ)が大きな
  ...
  強制的にクリックする)で行っています。
- 見出しは `# `(段落の先頭)のみサポートしています。実際に公開済みの記事の
  保存データ(devtoolsで確認)では、本文中の見出しは `<h3>...</h3>` として
  保存されていることを確認済みで、それに合わせて `editor.insertHTML()` で
  `<h3>` タグを直接挿入する実装にしていますが、この挿入方法自体
  (見出し以外の段落は改行`<br><br>`のみで区切り、実際の段落(`<div>`)分割を
  行わない一方、見出しだけは`<h3>`という別のブロック要素を挿入する形)は
  未検証です。うまく見出しにならない場合は、失敗時/`--inspect`時に保存される
  HTML(`error_form.html`など)を見せてもらえれば調整します。箇条書き・番号
  リストなど、それ以外のMarkdown記法はサポートしていません(そのまま文字として
  入力されます)。
...
- 予約投稿は実行時点から30日後までしか設定できないようです(サイト側の仕様。
  ドキュメントで明文化されているわけではなく、実際の挙動に基づく制限です)。
  それを超える日時は、フォーム入力前にエラーとして検出します。
...
```
posts/
  tags.txt           ← このフォルダ(=1つのブログ)の全記事に共通の興味関心タグ
                        (省略可。1行に1つずつ書く。記事ごとのtags.txtがあればそちらが優先)
  2026-09-10-recap/
    ...
    tags.txt         ← この記事だけの興味関心タグ(省略可。1行に1つずつ書く。上のposts/tags.txtより優先)
  2026-09-11-preview/
...
- **記事ごとに違う予約日時にしたい場合**は、そのフォルダに `publish_at.txt` を
  ...
  関係なく)必ずその日時で予約投稿になります。予約投稿は実行時点から30日後までしか
  設定できないようなので、1か月以上先の日付が混ざっている場合、その記事だけ
  エラーになります(他の記事の投稿は止まらずに続きます)。
...
- 興味関心タグに既定値はありません。次の優先順位で解決されます:
  1. 記事ごとの `tags.txt`(そのフォルダにあれば最優先)
  2. `--batch-dir` で指定したフォルダ直下の `tags.txt`(そのブログの全記事に共通のタグ)
  3. コマンドラインの `--tag` / `--no-tags`
  - どの記事にもどれも無い(該当する `tags.txt` も無く、`--tag`/`--no-tags` も未指定)場合は、
    どのフォルダにタグが足りないかを教えてくれるエラーになります。タグを付けたくない記事は、
    中身が空の `tags.txt` を置くか `--no-tags` を指定してください。
...
- `tags` は必須です(既定値は無いため)。タグを付けたい場合はタグ名の配列を、
  付けない場合は空配列 `[]` を指定してください。
...
投稿先は同じ文春野球友の会サイト(yakyu.bunshun.jp)の新規投稿フォームなので、
ブログ(記事シリーズ)ごとに投稿用フォルダ(または `--batch` のJSON)を分ければ、
複数のブログを並行して自動投稿できます。既存の「IT野球選手名鑑」用フォルダ
(`archiver/create/`)はそのままに、別のブログ用フォルダを新しく作るだけです。
...
```bash
# IT野球選手名鑑(既存どおり)
python post_blog.py --login-url https://yakyu.bunshun.jp/login --batch-dir archiver/create

# 別のブログ
python post_blog.py --login-url https://yakyu.bunshun.jp/login --batch-dir archiver/create-another-blog
```

- ブログごとにフォルダを分けておけば、そのフォルダ直下の `tags.txt` がそのブログの
  既定タグとして使われるため、コマンドラインで毎回 `--tag` を指定し直す必要はありません。
- ブログの種類が増えても `post_blog.py` 自体は共通のまま使い回せます(投稿先URL・
  ログイン方法・フォーム構造はどのブログでも同じ新規投稿フォームのためです)。

### 複数のブログのデータを1つのzipファイルで受け取る(`--batch-zip`)

他の人からブログの記事データ一式(複数ブログ分)をまとめてもらう場合など、
フォルダを個別に用意する代わりに、**1つのzipファイルにまとめて渡す**ことも
できます。zipの直下にブログ(記事シリーズ)ごとのフォルダを並べてください
(各フォルダの中身は、上記の `--batch-dir` と全く同じ規約です)。
...
```bash
python post_blog.py --login-url https://yakyu.bunshun.jp/login --batch-zip bundle.zip --draft
```

- zipは実行時に一時フォルダへ自動展開され、直下のフォルダをブログ単位として
  順番に(ブログもその中の記事も、フォルダ名の昇順で)まとめて投稿します。
  ログインは最初の1回だけです。全記事の投稿が終わると、展開に使った一時
  フォルダは自動的に削除されます。
- 記事ごとの予約日時・タグの優先順位は `--batch-dir` と同じです(記事フォルダの
  `tags.txt`/`publish_at.txt` → ブログフォルダ直下の `tags.txt` → コマンドラインの
  `--tag`/`--draft`等の順)。
- 1つのブログだけをzip化した場合(記事フォルダがzip直下に直接並んでいる場合)も、
  自動的に検出して1ブログ分として扱います。
- `--batch`/`--batch-dir` とは同時に指定できません。

### ChatGPTに書いてもらう(執筆ルール)

どのブログも記事本文・挿絵・タグまでChatGPT等に考えて出力してもらう想定です。
出力がこのツールの想定する形式(文字装飾のタグ、画像の挿入方法など)からズレると、
自動投稿時に書式が崩れたり画像が入らなかったりするため、
`archiver/WRITING_RULES_COMMON.md`(出力フォーマットの共通ルール。タイトル行の
形式、段落の区切り方、`<b>`/`<i>`/`<u>` による文字装飾、引用、画像の挿入方法・
生成、タグの選び方、出力するzipの構成、などを定義)をChatGPTへの指示の冒頭に
そのまま貼り付け、その後ろに書いてほしい記事のテーマ・タイトルの方向性・トーン
など、その記事固有の指示を続けて書いてください。ブログごとに別ファイルを用意
する運用はしていません(全ブログでこの共通ルールだけを使い回します)。

コード実行機能・画像生成機能が使えるChatGPT(Plus/Team等)であれば、
`article.txt`・本文中で参照した挿絵・`tags.txt` を1つのzip(サブフォルダなし)に
まとめて出力してもらえます。そのzipをダウンロードして、そのまま
`--batch-zip` に渡せば投稿できます(上記「複数のブログのデータを1つのzip
ファイルで受け取る」を参照。1記事分のzipでもサブフォルダに包まず
そのまま渡せます)。コード実行機能が無い場合は、出力されたテキストを
`article.txt` として、生成された画像を個別に記事フォルダへ保存して使って
ください(1行目=タイトル、2行目以降=本文。詳しくは上記「1. 本文ファイルを
用意する」/「3-a. フォルダで指定する」を参照)。
