...
- `**太字**`・`*斜体*`・`__下線__`(旧記法)も後方互換のため引き続き使えるが、
  **ChatGPT等に生成させる場合は`<b>`/`<i>`/`<u>`タグを使うことを推奨**する
  (標準的なMarkdownでは`__`も太字を意味し「下線」という概念自体が無いため、
  ChatGPTに`__下線__`と書かせても太字として解釈され、下線が反映されないことが
  実際にあった)。
- `<br>`(段落内改行)にも対応している。
- 段落と段落の間(空行区切り)は、実際のブログでは常に**空行1行分**の間隔になる
  (以前は実装の都合でEnterキーを2回押しており、空行2行分になってしまう不具合が
  あったため修正済み)。
- 引用(`> `)が複数行にわたる場合、行の区切りは自動的に`<br>`に変換されるので、
  `article.txt`側で行を分けて書けばそのまま改行として表示される。継続行の先頭に
  単独の`>`だけを書くと、その前後が「1行分の空き」として表示される。
...
- 興味関心タグは既定で「選手名鑑」「IT野球選手名鑑」が設定されます。実際のUIは
  「興味関心タグ」欄の「編集する」ボタン→開いたモーダル内の検索ボックスにタグ名を
  入力して絞り込み→完全一致するカードをクリック→「完了」で確定、という流れに
  なっており(似た名前のタグ、例:「偏愛選手名鑑2023」も候補に出るため完全一致で
  選ぶ必要がある)、この一連の操作を自動で行います。検索してもタグが候補に
  出てこない場合は指定しても設定できません。変えたい場合は
  `--tag タグ名`(複数指定可、指定した分だけが使われる)、付けたくない場合は
  `--no-tags` を指定してください。
  - それでも設定できない場合は、`--inspect-out`(既定 `archive/_new_post_inspect/`)に
    保存される `tags_debug.png` / `tags_debug.html`(タグ設定を試みた直後の画面)を
    見せてもらえれば、実際の構造に合わせて調整できます。
...
posts/
  2026-09-10-recap/
    ...
    tags.txt         ← この記事だけの興味関心タグ(省略可。1行に1つずつ書く)
  2026-09-11-preview/
...
- 興味関心タグは既定で「選手名鑑」「IT野球選手名鑑」が全記事に設定されます。
  記事ごとに変えたい場合はそのフォルダに `tags.txt` を置いてください。コマンド
  ライン全体の既定値を変えたい/無しにしたい場合は `--tag` / `--no-tags` を使います。
