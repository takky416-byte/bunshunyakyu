...
- 興味関心タグは既定で「選手名鑑」「IT野球選手名鑑」が設定されます(新規投稿フォームの
  「おすすめの興味関心タグ」に表示されるカードをテキストでクリックする方式のため、
  おすすめに出てこないタグは指定しても設定できません)。変えたい場合は
  `--tag タグ名`(複数指定可、指定した分だけが使われる)、付けたくない場合は
  `--no-tags` を指定してください。
  - タグ欄の正確なDOM構造は未確認のまま実装しているため、設定できたか怪しい場合は
    `--inspect-out`(既定 `archive/_new_post_inspect/`)に保存される
    `tags_debug.png` / `tags_debug.html`(タグ設定を試みた直後の画面)を見せてもらえれば、
    実際の構造に合わせて確実な方式に直せます。
