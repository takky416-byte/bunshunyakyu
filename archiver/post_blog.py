...
([html]) => {
    ...
    // 戻すためのクリック)でカーソル/選択範囲が本文の末尾以外の場所に
    // ズレていると、次の insertHTML() がその選択範囲を上書き(置き換え)
    // してしまい、既に入れたはずの内容が消えたり順番が入れ替わったりする
    // 不具合が実機テストで見つかった。挿入前に必ずカーソルを本文の一番
    // 最後へ強制的に戻してから挿入することで防ぐ。
    // 以前は[1e9, 1e9]のような大きすぎる位置を渡せば自動的に本文末尾に
    // 丸められることを期待していたが、実機では丸められず、直前にクリック
